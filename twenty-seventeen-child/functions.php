...
function show_past_event ( $category="", $start_date="1/1/2017", $num_post=3 ) {
        ...
        $time = strtotime($start_date. ' '. $timezone);
        ...
        foreach ( $events as $post ) {
                ...
                // Skip this event
                // if event is a recurring event AND
                // has not passed
                // below if condition check if event has passed or not only if event is multiple/recurring - commenting if conditn .

                // if (tribe_is_recurring_event($post->ID) && !is_past_event_in_recurring($post->ID)) {
                //     continue;
                // }

                // if event is recurring or not , it needs to be checked if event date is greater than current data -
                if(!is_past_event_in_recurring($post->ID)){
                    continue ;
                }
                ...
                echo '<div class="event-container margin-top-50">';
                  ...
                  echo '<div id="event-subcontainer3" class="margin-left-20">';
                    echo '<span style="font-size: 13px; font-weight: 700">'.show_event_dates($post->ID, false, true).'</span>';
 ...
 */
function show_recurring_dates ( $event_id, $is_home=false, $is_past=false ) {
        ...
        if ($is_home) {
                ...
                foreach ($dates as $date) {
                    ...
                    // we are comparing events using UTC time zone since now time is UTC, hence adding UTC 8 hours to $date_text since event dates are in LA...
                    // after comparison, we are rendering LA timezone date

                    if (strtotime($date. ' '. $timezone) > strtotime("now")) {
                        if (!$is_past) {
                            echo '<span class="tribe-event-date-start">'.date('F j \@  g:i a', $date_text).'</span><br>';
                        }
                    }
...
/**
*    This function is for the recurring events that checks if the event date has passed or not
*    $timezone = 'America/Los_Angeles';
*    $time = strtotime($start_date. ' '. $timezone;
*/
function is_past_event_in_recurring ( $event_id ) {
    $timezone = 'America/Los_Angeles';
    $dates = tribe_get_recurrence_start_dates( $event_id );

    //echo("dates"); //testing
    //var_dump($dates); // testing purpose


    $dateObj = strtotime($dates[0]. ' '. $timezone); // this prints tha time in Los Angeles timezone - change added by
    //this solved the problem when there are multiple events and the first event goes to past event on the day of event because of UTC effect..
    // but still first event is not shown in the upcoming events

    //testing purpose echo
    //$dateObj = strtotime($dates[0]);    // this prints tha time in UTC timezone- hence wrong-
    //echo("dateObject");
    //ar_dump($dateObj); // this will print unix timestamp with Los Angeles timezone in effect
    //echo("strtotime(now)");
    //echo(strtotime("now"));  // testing purpose
...
function show_event_dates ( $event_id, $is_home=false, $is_past=false ) {
        echo '<div class="event-date-container">';
        if ( tribe_is_recurring_event($event_id) ) {
                show_recurring_dates( $event_id, $is_home, $is_past );
        } else {
                // Single day event
...
function load_custom_js_assets() {
// this is pixel integration s
        echo '<script>
        ...
        // echo '<script src="' . get_stylesheet_directory_uri() . '/custom-javascript/pixel.js"></script>' . "\n";
        //wp_enqueue_script('pixel-integration-for-tracking-campaign', get_stylesheet_directory_uri() . '/custom-javascript/pixel.js', '','',true);

}

add_action('wp_footer', 'load_custom_js_assets',10);

function pixel_javascript() {

        echo '<noscript><iframe src="//s.thebrighttag.com/iframe?c=t4tmU86" width="1" height="1" frameborder="0" scrolling="no" marginheight="0" marginwidth="0"></iframe> </noscript>';

    }

add_action( 'wp_footer', 'pixel_javascript' );
...
$family_friendly = new Category(array(
        "logo_url" => '',
        "banner_url" => '',
        "logo_css_margin_left" => '',
        "logo_css_margin_right" => '',
        "color" => '',
        "logo_css_margin_event_list" => '',
        "logo_css_margin_past_event_list" => '',
        "sub_brand_logo_url" => 'http://hammertheatre.staging.wpengine.com/wp-content/uploads/2018/01/FamilyFriendlySeries_425x35.jpg',
        "sub_brand_url" => ''
));
...
$art_tech = new Category(array(
        ...
        "sub_brand_logo_url" => 'http://hammertheatre.staging.wpengine.com/wp-content/uploads/2018/07/425x35_arttech.png',
...
$lights_action = new Category(array(
        ...
        "logo_css_margin_past_event_list" => 'margin-top: 1%; margin-left: 15%; max-width: 2%;',
        "sub_brand_logo_url" => 'http://hammertheatre.staging.wpengine.com/wp-content/uploads/2018/07/425x35_lights_action.png',
...
$music_wo_borders = new Category(array(
        ...
        "sub_brand_logo_url" => 'http://hammertheatre.staging.wpengine.com/wp-content/uploads/2018/07/425x35_musicwoborders.png',
...
$holidays_at_hammer = new Category(array(
        ...
        "logo_css_margin_event_list" => 'margin-top: 1%; margin-left: 16.6%; max-width: 2%;',
        "logo_css_margin_past_event_list" => 'margin-top: 1%; margin-left: 15%; max-width: 2%;',
        "sub_brand_logo_url" => 'http://hammertheatre.staging.wpengine.com/wp-content/uploads/2018/07/425x35_holidays@hammer.png',
...
$hammer_speaks = new Category(array(
        ...
        "sub_brand_logo_url" => 'http://hammertheatre.staging.wpengine.com/wp-content/uploads/2018/07/425x35_hammerspeaks.png',
...
$sjsu_at_hammer = new Category(array(
        ...
        "logo_css_margin_event_list" => 'margin-top: 1%; margin-left: 16.6%; max-width: 2%;',
        "logo_css_margin_past_event_list" => 'margin-top: 1%; margin-left: 15%; max-width: 2%;',
        "sub_brand_logo_url" => 'http://hammertheatre.staging.wpengine.com/wp-content/uploads/2018/07/425x35_sjsu@hammer.png',
...
$also_at_hammer = new Category(array(
        ...
        "sub_brand_logo_url" => 'http://hammertheatre.staging.wpengine.com/wp-content/uploads/2018/07/425x35_also@hammer.png',
        "sub_brand_url" => '/also-hammer'
));


$hammer_news = new Category(array(
        "logo_url" => '',
        "banner_url" => '',
        "logo_css_margin_left" => '',
        "logo_css_margin_right" => '',
        ...
        "logo_css_margin_event_list" => '',
        "logo_css_margin_past_event_list" => '',
        "sub_brand_logo_url" => 'http://hammertheatre.staging.wpengine.com/wp-content/uploads/2018/03/425x35_hammernews.png',
        "sub_brand_url" => 'Error'
));
