...
$todays_date_time = new DateTime('now');
$todays_date_time->setTimezone(new DateTimeZone('America/Los_Angeles')); // +04

// Retrieve the next 4 upcoming events
$events = tribe_get_events( array(
   'posts_per_page' => 4,
   'start_date' => $todays_date_time
) );
...
foreach ( $events as $post ) {
    ...
    $logo_style_text = get_logo_css_margin ( $category_text, $is_left );
        ...
        echo '<h3 class="margin-bottom-20 home-event-series"><a href="'.$brand_url.'">'.$category_text.'<span style="color: var('.tribe_get_color_for_categories( tribe_get_text_categories() ).');">&#x2794;</span></a></h3>';
        ...
        echo '<p id="home-event-date-'.$counter.'" class="margin-bottom-20">'.show_event_dates($post->ID, true).'</p>';
        ...
        $ticket_url_reserved = get_post_meta($post->ID, 'Ticket_Link_Reserved', true);
        if($ticket_url === "Free") { 
        	 echo '<span class="home-event-button" style="margin: 0 auto;"><h4 class="free-event" style="text-align: center; margin-top: 5px; font-size: 1.6rem; font-weight: 400; font-weight: bold;">This is a free event.</h4></span>';
        }
        else if($ticket_url === "Reserved") {
        	echo '<a class="home-event-button button buy-ticket-button-container" href="'.$ticket_url_reserved.'">RESERVE TICKETS</a>';
        }
        else if($ticket_url) {
        ...
        // Render ticket link in single event page
      	...
      	if ($ticket_type == 'free') {
        	...
        	if ($ticket_type == 'reserved') {
        		echo '<a class="home-event-button button buy-ticket-button-container" href="'.$ticket_url.'">RESERVE TICKETS</a>';
        	} else {
        		echo '<a class="home-event-button button buy-ticket-button-container" href="'.$ticket_url.'">BUY TICKETS</a>';
     		} 
		}
